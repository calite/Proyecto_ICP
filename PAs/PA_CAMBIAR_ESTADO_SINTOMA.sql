...
AS
	...
	BEGIN TRY
		...
		END
				...
				IF ISNULL(@ID_REPARACION_ESTADO,'') = ''
		BEGIN
			SET @RETCODE = 10
		...
		IF @NTRANS = 0 BEGIN TRANSACTION [TR_CAMBIAR_ESTADO_SINTOMA]		
			...
			--UPDATE REPARACIONES SET ID_ESTADO = @ID_ESTADO WHERE ID_REPARACION = @ID_REPARACION --TEST
			UPDATE REPARACIONES_ESTADO SET ID_ESTADO = @ID_ESTADO WHERE ID_REPARACION_ESTADO = @ID_REPARACION_ESTADO AND ID_REPARACION = @ID_REPARACION
